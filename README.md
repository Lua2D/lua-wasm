...
- **AOT for static code (being retired).** Modules known when the artifact is built can be compiled ahead of time to C by `luaot` and linked in. Measured across four browser engines, the wasm payoff proved marginal on the workloads a real embedder runs — ~1.1–1.2× on V8 (Chrome) and JSC (Safari), and a net *loss* on SpiderMonkey (Firefox) — with a clear win only for textbook numeric loops that no known consumer runs. On that evidence AOT is being removed, and **this is the last release to include it.** The measurements and the reasoning are in [`experiments/RESULTS.md`](experiments/RESULTS.md) and [`doc/aot-sunset-2026-07-24.md`](doc/aot-sunset-2026-07-24.md). (Game-shaped code — tables, allocation, GC — always measured ~1×, since AOT does not touch that cost.)
